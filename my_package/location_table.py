...
class LocationTable:
    ...
    def export_to_parquet(self, gdf, file_name):
        ...
        '''
        gdf.to_parquet(self.path+"output/"+file_name+".parquet", compression='uncompressed', index=False)
        print("Location table extracted")

    def export_to_csv(self):  # TODO
        # Export to csv
        gdf.to_csv('production_table_20200630.csv', sep='|', encoding='utf-8', index=False)
